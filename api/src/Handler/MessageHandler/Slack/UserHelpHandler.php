...
class UserHelpHandler
{
    public function showUserHelp(SlashCommand $command)
    {
        ...
        $m->addTextSection('You can mention me using @TimeMe in the hello-bye channel and put `/hi` or `/bye` into the message.');
        $m->addTextSection(':wave: For example "Good morning everyone `/hi @TimeMe`" would sign you in for the day and start your work timer.');
        $m->addTextSection(':runner: "Bye bye everyone `/bye @TimeMe`" would stop all your work timers and sign you out for the day.');
        ...
        $m->addTextSection(':clock9: `/end_work` will start a work timer for you. You will only need this if you do *not* use the /hi and /bye commands.');
